...
def header(active):
  ...
  <div class="wrap">
    <div class="logo"><a href="/">%(title)s</a></div>
    %(nav)s
    %(social)s
...
MODAL = """<div class="modal" id="modal" role="dialog" aria-modal="true" aria-hidden="true">
  ...
  <button class="modal-close" type="button" aria-label="Close">
    <svg viewBox="0 0 18 18" aria-hidden="true"><path d="M18,15.141 L11.76,8.98 L17.91,2.77 L15.14,0 L8.98,6.24 L2.75,0.09 L0,2.84 L6.24,9.02 L0.09,15.25 L2.84,18 L9.01,11.76 L15.23,17.91 L18,15.141 Z"/></svg>
  </button>
  <div class="modal-nav">
    <button class="m-prev" type="button" aria-label="Previous project">
      <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M12.7 1.3 4 10l8.7 8.7 1.4-1.4L6.8 10l7.3-7.3z"/></svg>
    </button>
    <button class="m-next" type="button" aria-label="Next project">
      <svg viewBox="0 0 20 20" aria-hidden="true"><path d="M7.3 1.3 5.9 2.7 13.2 10l-7.3 7.3 1.4 1.4L16 10z"/></svg>
    </button>
  </div>
</div>"""
...
def tile(post):
    caption = strip_tags(post["body"])[:150]
    alt = "%s — %s" % (post["title"], post["subtitle"]) if post["subtitle"] else post["title"]
    return """    <a class="tile" href="/work/%(slug)s/"
       data-slug="%(slug)s"
       data-title="%(title)s"
       data-subtitle="%(subtitle)s"
       data-full="/assets/img/%(slug)s-960.jpg"
       data-fullset="%(full_srcset)s"
       aria-label="%(aria)s">
      <picture>
      ...
      <template>%(body)s</template>
        ...
        "subtitle": esc(post["subtitle"]),
        "webp": srcset(post, "webp"),
        "full_srcset": srcset(post, "webp"),
        "alt": esc(alt),
        "aria": esc("%s, %s" % (post["title"], post["subtitle"] or caption[:60])),
        "w": post["w"], "h": post["h"],
        "body": post["body"],
    }
